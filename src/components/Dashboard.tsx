...
const Dashboard = React.forwardRef<HTMLDivElement>((props, ref) => {
  const { userEmail, username, userSub, attributes, signOut } = useAuth();
  ...
  const [imeiSource, setImeiSource] = useState<"storage" | "username" | "cognito" | "manual" | "dvd" | undefined>(
    imei ? "storage" : undefined,
  );
  const {
    vehicleData,
    vehicleInfo,
    ...
    return (
      <div className="min-h-screen bg-background p-4 flex items-center justify-center">
        <div className="glass-card p-8 text-center max-w-md">
          ...
          <h2 className="font-display font-bold text-xl text-foreground mb-2">Erreur de connexion</h2>
      ...
      <div className="max-w-7xl mx-auto p-3 md:p-4 space-y-3 md:space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="glass-card p-3 md:p-4">
            ...
            <p className="font-display text-sm md:text-lg font-semibold text-foreground">{displayImmat ?? "—"}</p>
          </div>
        ...
        <VehicleMap
          ...
          className="h-[35vh] md:h-[40vh] min-h-[250px] md:min-h-[300px] shadow-card"
          isPrivate={isPrivate}
        />
          ...
          {loading && !vehicleData ? (
            ...
            <>
              <MetricCard
                ...
                unit="km/h"
                variant={
                  (vehicleData?.speed ?? 0) > 120 ? "danger" : (vehicleData?.speed ?? 0) > 80 ? "warning" : "success"
                }
              />
        ...
        <div className="glass-card p-4">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="font-display font-semibold text-foreground">Position GPS</h2>
          </div>
          {loading && !vehicleData ? (
